...
public class Polynomial implements Comparable<Polynomial> {
	...
	private static final class ReverseComparator implements
	...
	public boolean isIrreducible() {
		return this.isIReducible();
	}

	/**
	 * Constructs a polynomial using the bits from a String in the format x^m +
	 * ... + 1
	 */
	public static Polynomial createFromString(String pol) {
		String toSplit = pol.replace("x^", "");
		String[] parts = toSplit.split("[+]");

		int[] arrayToConcat = new int[parts.length];

		for (int i = 0; i < parts.length; i++) {
			String temp = parts[i].trim();
			Integer toConcat = Integer.parseInt(temp);
			arrayToConcat[i] = toConcat.intValue();
	...
	public Polynomial mod(Polynomial that) {
		...
		for (BigInteger i = da.subtract(db); i.compareTo(BigInteger.ZERO) >= 0; i = i
	...
	public BigInteger toBigIntegerAccurate() {
		...
		for (BigInteger degree : degrees) {
			...
			for (BigInteger i = BigInteger.ONE; i.compareTo(degree) <= 0; i = i
					.add(BigInteger.ONE)) {
	...
	public BigInteger getA() {
		Iterator<BigInteger> iterator = this.degrees.descendingIterator();
		BigInteger temp = iterator.next();
		BigInteger toReturn = null;
		while (temp.compareTo(this.degree()) != 0) {
			toReturn = temp;
			temp = iterator.next();
		}
		return toReturn;
